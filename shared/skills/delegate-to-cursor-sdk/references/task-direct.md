...
Use this template for small, clear, low-risk tasks where no subagent plan is needed. Materialize only the fenced packet body below into the current invocation's marker-owned `packetsDir` from `owned-artifact-cleanup.md`, then replace every placeholder before dispatch. Never copy the explanatory wrapper or edit this source template in place.
